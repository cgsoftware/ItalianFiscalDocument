...
class FiscalDocHeader(osv.osv):
   ...
   _columns = {
             ...
             'partner_indfat_id': fields.many2one('res.partner', 'Indirizzo di Fattura'),
             'partner_indcons_id': fields.many2one('res.partner', 'indirizzo di Consegna'),
             ...
             'banca_azienda':fields.many2one('res.bank', 'Banca Azienda', required=False, help="Banca del Azienda "),
   ...
   def onchange_partner_id(self, cr, uid, ids, part, context):
        ...
        addr = self.pool.get('res.partner').address_get(cr, uid, [part], ['delivery', 'default'])
...
#       fiscal_position = part.property_account_position and part.property_account_position.id or False
        ...
        if part.bank_ids:
            ...
            banca_cliente = False
        val = {
            'partner_indfat_id': addr['default'],
...
class FiscalDocRighe(osv.osv):
  ...
  # dp.get_precision('Account')
   ...
   _columns = {
             ...
             'product_uom_qty': fields.float('Quantity (UoM)', digits_compute= dp.get_precision('Product UoS'), required=True, readonly=False , Traslate=True),
             'product_uom': fields.many2one('product.uom', 'Unit of Measure ', required=True, readonly=False, Traslate=True),
             'product_uos_qty': fields.float('Quantity (UoS)',digits_compute= dp.get_precision('Product UoS'), readonly=False, Traslate=True),
 ...
 #               tot_merce = tot_merce-(tot_merce*riga.name.sconto_pagamento/100)
   ...
   def determina_prezzo_sconti(self, cr, uid, ids, product_id, listino_id, qty, partner_id, uom, data_doc):
            # determina il prezzo
                ...
                #TODO DA VERIFICARE!!! MOD PER CLIENTE NONNO FELICE
                #~ else:
                    #~ if list_price[listino_id] != new_list_price and  sconto > 0:
                        #~ new_list_price = list_price[listino_id]
...
class FiscalDocIva(osv.osv):
    ...
    _columns = {
             ...
             'imposta':fields.float("Totale Imposta", digits_compute=dp.get_precision('Account')),
               }
    _rec_name = 'name'
    ...
    def agg_righe_iva(self, cr, uid, ids, context):
        ...
        company_id = self.pool.get('res.users').browse(cr, uid, uid, context).company_id.id
        codici_iva_accessori = self.pool.get('res.company').read(cr, uid, company_id , (['civa_spe_inc', 'civa_spe_imb', 'civa_spe_tra', 'civa_fc', 'civa_bolli']), context=context)
        ...
        #aggiorna il castelletto con le righe iva per i bolli
        if testata.totale_bolli:
            if codici_iva_accessori['civa_bolli'][0] or esenzione:
                if esenzione:
                    if righe_iva.get(esenzione.id, False):
                        # esiste gia la riga con questo codice
                        dati_iva = righe_iva[esenzione.id]
                        dati_iva['imponibile'] = dati_iva['imponibile'] + testata.totale_bolli
                        righe_iva[esenzione.id] = dati_iva
                    else:
                        dati_iva = {'imponibile': testata.totale_bolli}
                        righe_iva.update({esenzione.id:dati_iva})
                else:
                    if righe_iva.get(codici_iva_accessori['civa_bolli'][0], False):
                        dati_iva = righe_iva[codici_iva_accessori['civa_bolli'][0]]
                        dati_iva['imponibile'] = dati_iva['imponibile'] + testata.totale_bolli
                        righe_iva[codici_iva_accessori['civa_bolli'][0]] = dati_iva
                    else:
                        dati_iva = {'imponibile': testata.totale_bolli}
                        righe_iva.update({codici_iva_accessori['civa_bolli'][0]:dati_iva})
